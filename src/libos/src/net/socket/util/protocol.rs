...
impl SocketProtocol {
    pub fn check_datagram_proto(self) -> Result<Self> {
        match self {
            SocketProtocol::IPProtocol(ip_protocol) => {
                if ip_protocol != IPProtocol::IPPROTO_IP && ip_protocol != IPProtocol::IPPROTO_UDP {
                    ...
                    Ok(SocketProtocol::IPProtocol(ip_protocol))
                }
            }
            SocketProtocol::NetlinkFamily(netlink_family) => {
                Ok(SocketProtocol::NetlinkFamily(netlink_family))
            }
            SocketProtocol::EthernetProtocol(ethernet_protocol) => {
                Ok(SocketProtocol::EthernetProtocol(ethernet_protocol))
            }
        }
    }

    pub fn check_stream_proto(self) -> Result<Self> {
        match self {
            SocketProtocol::IPProtocol(ip_protocol) => {
    ...
    pub fn check_rawsock_proto(self) -> Result<Self> {
        match self {
            SocketProtocol::IPProtocol(ip_protocol) => {
