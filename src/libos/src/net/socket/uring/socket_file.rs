...
impl SocketFile {
    pub fn new(
        ...
        match socket_type {
            SocketType::STREAM => {
                let protocol = SocketProtocol::check_stream_proto(protocol).unwrap();
            ...
            SocketType::DGRAM => {
                let protocol = SocketProtocol::check_datagram_proto(protocol).unwrap();
                let any_socket = match domain {
                    Domain::INET => {
            ...
            SocketType::RAW => {
                let protocol = SocketProtocol::check_rawsock_proto(protocol).unwrap();
                let any_socket = match domain {
                    Domain::INET => {
